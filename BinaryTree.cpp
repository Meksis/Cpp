...
Branch* del_elem(Branch*& aBranch, T aData) {
	...
	{
    	Branch* tmp;
    	if (Root->RightBranch == NULL) tmp = Root->LeftBranch;
    	else {
    	...
    	else if (aData < Root->Data) Root->LeftBranch = del_elem(Root->LeftBranch, aData);
    	else Root->RightBranch = del_elem(Root->RightBranch, aData);
    	return Root;
    }
...
int main()
{
    ...
    int number, value, vel, element, k;
    list<int> a;
    Branch<int>* Root = 0;
    Branch D(Root);
    
    cout << "Количество Элементов: "; cin >> vel; cout << endl;
    
    for (int i = 0; i < vel; i++)
    {
        value = rand() % 100;
        cout << "Добавление - " << value << endl; D.Add(value);
    }
    
    cout << "Элементы дерева - " << endl; D.print();
    
    cout << "Элемент для Добавления:" << endl; cin >> element; D.add_elem(element);
    
    cout << "Элементы дерева - " << endl; D.print();
    
    cout << "Элемент для поискаа:" << endl; cin >> number;
    
    cout << "Заменить на: " << endl; cin >> value; D.change(number, value);
